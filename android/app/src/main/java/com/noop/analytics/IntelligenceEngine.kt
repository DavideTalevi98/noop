...
object IntelligenceEngine {
    ...
    /**
     * Serialises [analyzeRecent] against itself. The pass is launched from three independent coroutines ,
     * the 15-min backstop loop and rescoreAfterEdit (both AppViewModel), and the post-offload analyze
     ...
     * `computing` guard (IntelligenceEngine.swift:287), but SERIALISES rather than coalesces on purpose:
     * Android's callers pass heterogeneous windows , the Effort rescore uses maxDays=4000, not 21, and can
     * overlap the *independent* BLE-offload analyze (unlike Swift, which awaits it before the loop). A
     * drop-guard would skip that full-history rescore while its unconditional flagSet marks it permanently
     * done, and would re-run the holder's 21-day window in its place. withLock lets every caller run its OWN
     * pass, queued and never parallel , so nothing is dropped and no window is silently lost. Suspending
     * (not thread-blocking) and cancellation-cooperative, matching the callers' #125 CancellationException
     * handling. No re-entrancy: nothing analyzeRecent calls re-enters it.
     */
